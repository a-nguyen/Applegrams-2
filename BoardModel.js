var Board = Backbone.Model.extend({
  ...
  initialize: function () {
    ...
    this.removePiece = function (x, y) {
      ...
      this.matrix(x, y, 0);
      this.colorMatrix[y][x] = 0;
      this.colorWords(x+1, y);
      this.colorWords(x-1, y);
      this.colorWords(x, y-1);
      this.colorWords(x, y+1);
      return piece;
    };
    ...
    this.green = function (x, y) {
      if (x < 20 && x > -1 && y < 20 && y > -1) {
        this.colorMatrix[y][x] = 1;
      }
    };

    this.ungreen = function (x, y) {
      if (x < 20 && x > -1 && y < 20 && y > -1) {
        this.colorMatrix[y][x] = 1;
      }
    };

    this.greenWord = function (word) {
      for (var i = 1; i < word.length; i++) {
        this.green(word[i][0], word[i][1]);
      }
    };

    this.ungreenWord = function (word) {
      for (var i = 1; i < word.length; i++) {
        this.ungreen(word[i]);
      }
    };

    this.colorWord = function (word) {
     if (this.checkWord(word)) {
        this.greenWord(word);
      } else {
        this.ungreenWord(word);
        
      }
    };

    this.colorWords = function (x, y) {
      var rowWord = this.grabRowWord(x, y);
      var colWord = this.grabColWord(x, y);
      this.colorWord(rowWord);
      this.colorWord(colWord);
    };
    ...
    this.colorMatrix = this.makeEmptyMatrix(20);
    for (i = 0; i < 7; i++) {
